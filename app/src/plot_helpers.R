...
add_temp_traces <- function(plt, df, yaxis = "y1", label = "", dash = F) {
  ...
  plt |>
    add_trace(
      ...
      line = list(
        color = OPTS$plot_colors$min_temp,
        width = width,
        shape = "spline",
        dash = dash
      )
    ...
    add_trace(
      ...
      hovertemplate = "%{y:.1f}°F",
      line = list(
        color = OPTS$plot_colors$mean_temp,
        width = width,
        shape = "spline",
        dash = dash
      )
    ...
    add_trace(
      ...
      hovertemplate = "%{y:.1f}°F",
      line = list(
        color = OPTS$plot_colors$max_temp,
        width = width,
        shape = "spline",
        dash = dash
      )
    ...
    add_trace(
      ...
      x = df$date,
      y = if_else(df$min_temp <= 32, 32, NA),
      yaxis = yaxis,
      type = "scatter",
      mode = "lines",
      hovertemplate = "Yes",
    ...
    add_trace(
      ...
      x = df$date,
      y = if_else(df$min_temp <= 28, 28, NA),
      yaxis = yaxis,
      type = "scatter",
      mode = "lines",
      hovertemplate = "Yes",
    ...
    add_trace(
      ...
      x = df$date,
      y = if_else(df$min_temp <= 24, 24, NA),
      yaxis = yaxis,
      type = "scatter",
      mode = "lines",
      hovertemplate = "Yes",
...
add_frost_freeze_kill_traces <- function(plt, df, yaxis = "y1") {
  ...
  plt |>
    ...
    add_trace(
      ...
      hovertemplate = "%{y:.1f}%",
      line = list(
        color = OPTS$plot_colors$freeze,
        width = width,
        shape = "spline"
      )
    ) |>
    add_trace(
      name = "Killing freeze probability",
      x = df$date,
      y = df$kill * 100,
      yaxis = yaxis,
      type = "scatter",
